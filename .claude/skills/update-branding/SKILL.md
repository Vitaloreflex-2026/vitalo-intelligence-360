...
2. **Place the files** in `src/components/atomic-crm/root/logos/`:
   - **Drop-in (simplest):** overwrite `logo_vitaloreflex_dark.png` / `logo_vitaloreflex_light.png`, keeping the exact filenames + extension. The imports in `defaultConfiguration.ts` already point here — done.
