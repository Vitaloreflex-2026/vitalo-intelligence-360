...
export const generateSales = (_: Db): Sale[] => {
  const randomSales = Array.from(Array(5).keys()).map((id) => {
    ...
    return {
      ...
      administrator: false,
      disabled: false,
      // Spread over the palette, mirroring the migration seed, so the calendar
      // tells the team apart without two consultants sharing a fill.
      color: RDV_PALETTE[(id + 1) % RDV_PALETTE.length],
    };
  });
  return [
    {
